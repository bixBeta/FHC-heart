...
meta_pavian = read.csv("global_Data/hear_meta_pavian.csv", header = T)
rownames(meta_pavian) = meta_pavian$simpleName

norm.bacteria.list = list()

for(i in 1:nrow(meta_pavian)){
  
  norm.bacteria.list[[i]] <- raw.matrix.bacteria[,i]/meta_pavian$Bacterial.reads[i]  
  
}

names(norm.bacteria.list) <- rownames(meta_pavian)

norm.bacteria.counts <- do.call("rbind", norm.bacteria.list)
colnames(norm.bacteria.counts) <- rownames(raw.matrix.bacteria)
input.matrix = t(norm.bacteria.counts)
colnames(input.matrix) <- colnames(raw.matrix.bacteria)
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
save(ctrl.matrix, acute.matrix, chronic.matrix, input.matrix, meta_pavian, raw.matrix.bacteria, samplemeta, file = "data/02__HEART__BACTERIA__all__matrices__norm__and__raw.Rdata")
...
#------------------------------------------------------------
save(res.acute, res.chronic, res.acute.v.chronic, p.vals.acute, p.vals.chronic, p.vals.acute.v.chronic, file = "results/bacteria__wilcoxon__results.Rdata")
#------------------------------------------------------------
