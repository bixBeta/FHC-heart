...
norm.virus.list = list()

for(i in 1:nrow(meta_pavian)){
  
  norm.virus.list[[i]] <- raw.matrix.virus[,i]/meta_pavian$Viral.reads[i]  
  
}

names(norm.virus.list) <- rownames(meta_pavian)

norm.virus.counts <- do.call("rbind", norm.virus.list)
colnames(norm.virus.counts) <- rownames(raw.matrix.virus)
input.matrix = t(norm.virus.counts)
colnames(input.matrix) <- colnames(raw.matrix.virus)
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
save(ctrl.matrix, acute.matrix, chronic.matrix, input.matrix, meta_pavian, raw.matrix.virus, samplemeta, file = "data/02__HEART__virus__all__matrices__norm__and__raw.Rdata")
...
#------------------------------------------------------------
save(res.acute, res.chronic, res.acute.v.chronic, p.vals.acute, p.vals.chronic, p.vals.acute.v.chronic, file = "results/virus__wilcoxon__results.Rdata")
#------------------------------------------------------------
